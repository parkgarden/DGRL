...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, case, env_paras, data_source='case'):
        ...
        self.ops = torch.arange(self.num_opes, device=self.device).view(1, 1, self.num_opes)
        '''
        features, dynamic
            ope:
                Status
    ...
    def step(self, actions):
        ...
        mask_not_completed = torch.ones(size=(self.batch_size, self.num_opes), dtype=torch.bool)

        # Removed unselected O-M arcs of the scheduled operations
        remain_ope_ma_adj = torch.zeros(size=(actions.shape[0],self.batch_size, self.num_mas), dtype=torch.int64)
        remain_ope_ma_adj.scatter_(dim=2, index=mas.unsqueeze(-1), value=1)
        ...
        #     self.ope_ma_adj_batch[self.batch_idxes, opes[i]] = remain_ope_ma_adj[i]
        
        batch_idx_expand = self.batch_idxes.unsqueeze(0).expand(actions.shape[0], self.batch_idxes.size(0)) 
        batch_idx_flat = batch_idx_expand.reshape(-1)                
        self.ope_ma_adj_batch[batch_idx_flat, opes.reshape(-1)] = remain_ope_ma_adj.reshape(-1, self.num_mas)

        # update processing times of uncompleted operations to prevent from missing processing time
        self.proc_times_batch[mask_not_completed] *= self.ope_ma_adj_batch[mask_not_completed]
        ...
        self.feat_opes_batch[self.batch_idxes, :3, opes] = torch.stack((torch.ones((actions.shape[0], self.batch_idxes.size(0)), dtype=torch.float),
                                                                        torch.ones((actions.shape[0], self.batch_idxes.size(0)), dtype=torch.float),
                                                                        proc_times), dim=2)
        # remove the scheduled operation's link of precedent operations 
        ...
        start_ope = self.num_ope_biases_batch[self.batch_idxes, jobs].unsqueeze(-1)
        end_ope = self.end_ope_biases_batch[self.batch_idxes, jobs].unsqueeze(-1)
        mask = (self.ops >= start_ope) & (self.ops <= end_ope)
        self.feat_opes_batch[self.batch_idxes, 3, :] -= mask.sum(dim=0).to(torch.float32)
        ...
        self.schedules_batch[self.batch_idxes, opes, :2] = torch.stack((torch.ones(actions.shape[0], self.batch_idxes.size(0)), mas), dim=2)
        # self.schedules_batch[mask_not_completed, 2] = self.feat_opes_batch[mask_not_completed, 5]
        # self.schedules_batch[mask_not_completed, 3] = (self.feat_opes_batch[mask_not_completed, 5] +self.feat_opes_batch[mask_not_completed, 2])
        
        self.schedules_batch[self.batch_idxes, :, 2] = torch.where(mask_not_completed[self.batch_idxes], self.feat_opes_batch[self.batch_idxes, 5, :], self.schedules_batch[self.batch_idxes, :, 2])
        self.schedules_batch[self.batch_idxes, :, 3] = torch.where(mask_not_completed[self.batch_idxes], self.feat_opes_batch[self.batch_idxes, 5, :] + self.feat_opes_batch[self.batch_idxes, 2, :], self.schedules_batch[self.batch_idxes, :, 3])

        self.machines_batch[self.batch_idxes, mas, 0] = torch.zeros(actions.shape[0], self.batch_idxes.size(0))
