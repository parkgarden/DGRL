...
class HGNNScheduler(nn.Module):
    ...
    def act(self, state, memories, dones, action_threshold, flag_sample=True, flag_train=True):
        ...
        num_j = state.mask_job_finish_batch.size(1)
        num_m = state.mask_ma_procing_batch.size(1)

        probs_reshaped = action_probs.squeeze().view(num_m, num_j)
        probs = action_probs.squeeze()

        # Apply action threshold: if 0, use greedy single-action; otherwise allow multi-action selection
        if action_threshold == 0:
            valid_mask = probs_reshaped > action_threshold
        else:
            valid_mask = probs_reshaped > (1 / action_threshold)
        ...
        if valid_mask.sum() <= 1:
            ...
            action_list.append(torch.stack((opes, mas, jobs), dim=1).t())
            return action_list

        # Multi-action selection: greedily pick (job, machine) pairs using bipartite matching
        # to avoid assigning the same job or same machine more than once
        selected = []
        NEG_INF = float('-inf')
        while True:
            ...
            masked_probs = probs_reshaped.clone()
            ...
            mas = (idx // state.mask_job_finish_batch.size(1))
            jobs = (idx % state.mask_job_finish_batch.size(1))
            opes = ope_step_batch[state.batch_idxes, jobs]

            selected.append(idx)
            action_list.append(torch.stack((opes, torch.tensor([mas]), torch.tensor([jobs])), dim=1).t())

            # Mask out chosen job column and machine row to enforce bipartite constraint
            valid_mask[mas, :] = False
            valid_mask[:, jobs] = False

            if len(selected) >= num_m:
                break
