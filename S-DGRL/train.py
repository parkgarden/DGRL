...
parser.add_argument('--n_j', type=int, default=10, help='Number of jobs of instances to train')
parser.add_argument('--n_m', type=int, default=5, help='Number of machines instances to train')
parser.add_argument('--dyn', type=int, default=0, help='dynamic graph')
...
def main():
    ...
    with open("./config.json", 'r') as load_f:
        ...
        load_dict["env_paras"]["num_jobs"] = params.n_j
        load_dict["env_paras"]["num_mas"] = params.n_m
        load_dict["env_paras"]["graph_len"] = params.graph_len
    ...
    env_valid = get_validate_env(env_valid_paras, params.dyn)  # Create an environment for validation
    ...
    for i in range(1, train_paras["max_iterations"]+1):
        ...
        if (i - 1) % train_paras["parallel_iter"] == 0:
            ...
            if params.dyn ==0:
                ...
                env = gym.make('fjsp-v3', case=case, env_paras=env_paras)
        ...
        if i % train_paras["save_timestep"] == 0:
            ...
            if vali_result < makespan_best:
                ...
                save_file = '{0}/save_best_{1}_{2}_{3}_dyn{4}_len{5}.pt'.format(save_path, num_jobs, num_mas, i, params.dyn, params.graph_len)
        ...
        # reward_log.append(mean_rewards_all_env)
        # loss_log.append(mean_rewards_all_env)
    ...
    # # Training curve storage path (value of each validating instance)
    # writer_100 = pd.ExcelWriter('{0}/training_100_{1}.xlsx'.format(save_path, str_time))
    with pd.ExcelWriter('{0}/training_ave_{1}_dyn{2}.xlsx'.format(save_path, str_time, params.dyn), engine="openpyxl") as writer:
    ...
    with pd.ExcelWriter('{0}/training_100_{1}_dyn{2}.xlsx'.format(save_path, str_time, params.dyn), engine="openpyxl") as writer:
        data.to_excel(writer)
    # print(data)
    ...
    # data = pd.DataFrame(np.array(reward_log).transpose(), columns=["reward"])
    # with pd.ExcelWriter('{0}/training_reward_{1}_dyn{2}.xlsx'.format(save_path, str_time, params.dyn), engine="openpyxl") as writer:
    #     data.to_excel(writer)
    # data = pd.DataFrame(np.array(loss_log).transpose(), columns=["loss"])
    # with pd.ExcelWriter('{0}/training_loss_{1}_dyn{2}.xlsx'.format(save_path, str_time, params.dyn), engine="openpyxl") as writer:
    #     data.to_excel(writer)
