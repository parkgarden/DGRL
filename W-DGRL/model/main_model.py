...
class DualAttentionNetwork(nn.Module):
    ...
    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx, sparse_mm):
        """
        ...
        # breakpoint()
        comp_idx_for_mul = comp_idx.reshape(sz_b, -1, J) #[sz_b, M*M, J]
        if sparse_mm == 0:  

        # breakpoint()
            for layer in range(self.num_dan_layers):
                candidate_idx = candidate.unsqueeze(-1). \
                    repeat(1, 1, fea_j.shape[-1]).type(torch.int64) # [sz_b, J, layer dimension]

                # fea_j_jc: candidate features with shape [sz_b, J, layer dimension]
                # breakpoint()
                fea_j_jc = torch.gather(fea_j, 1, candidate_idx).type(torch.float32)
                comp_val_layer = torch.matmul(comp_idx_for_mul,
                                        fea_j_jc).reshape(sz_b, M, M, -1) #[sz_b, M, M, layer dimension] 
                # comp_val_layer = torch.sparse.mm(
                #     comp_idx_for_mul_sparse[0],  # Sparse matrix
                #     fea_j_jc[0]          # Dense matrix
                # ).unsqueeze(0).reshape(sz_b, M, M, -1)
            ...
            comp_idx_for_mul_sparse = comp_idx_for_mul.to_sparse()
        ...
        fea_j_global = nonzero_averaging(fea_j) # [sz_b, output dimension(8)]
        fea_m_global = nonzero_averaging(fea_m) # [sz_b, output dimension(8)]
...
class DANIEL(nn.Module):
    ...
    def forward(self, fea_j, op_mask, candidate, fea_m, mch_mask, comp_idx, dynamic_pair_mask, fea_pairs, sparse_mm=0):
        """
        ...
        """
        # breakpoint()
        # job_remain_op_num = fea_j[0,:,7][candidate]

        # fea_standard_filter = job_remain_op_num
        # sorted_job_indices = fea_standard_filter.argsort(descending=True).squeeze()
