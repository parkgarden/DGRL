...
def load_data_from_files(directory, num_ins):
    # load_data_from_files(directory, num_ins)
    """
...
def load_data_from_files2(directory, ins_start, ins_end):
    """
        load all files within the specified directory
    :param directory: the directory of files
    :return: a list of data (matrix form) in the directory
    ...
    for root, dirs, files in os.walk(directory):
        ...
        files.sort(key=lambda s: int(re.findall("\d+", s)[0]))
        files.sort(key=lambda s: int(re.findall("\d+", s)[-1]))
        for f in files[ins_start:ins_end]:
        # for f in files:
            # print(f)
            g = open(os.path.join(root, f), 'r').readlines()
            job_length, op_pt = text_to_matrix(g)
...
def load_data_from_all_files(directory):
    """
        load all files within the specified directory
    :param directory: the directory of files
    :return: a list of data (matrix form) in the directory
...
def pack_data_from_config(data_source, test_data, num_ins):
    """
    ...
    for data_name in test_data:
        ...
        if num_ins == 0:
            data_list.append((load_data_from_all_files(data_path), data_name))
        else:
            data_list.append((load_data_from_files(data_path, num_ins), data_name))
    return data_list
    
def pack_data_from_config2(data_source, test_data, ins_start, ins_end):
    """
        load multiple data (specified by the variable 'test_data')
        of the specified data source.
    :param data_source: the source of data (SD1/SD2/BenchData)
    ...
    for data_name in test_data:
        data_path = f'./data/{data_source}/{data_name}'
        data_list.append((load_data_from_files2(data_path, ins_start, ins_end), data_name))
        # if num_ins == 0:
        #    data_list.append((load_data_from_all_files(data_path), data_name))
        # else:
        #    data_list.append((load_data_from_files(data_path, num_ins), data_name))
