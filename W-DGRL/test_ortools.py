...
def solve_instances(config):
    ...
    if not os.path.exists(f'./or_solution/{data_source}'):
        os.makedirs(f'./or_solution/{data_source}')

    # data_list = pack_data_from_config(data_source, config.test_data, num_ins)
    if params.test_data[0] == '0':
            test_data_input = ['']
    else:
        test_data_input = params.test_data
    data_list = pack_data_from_config(params.data_source, test_data_input, params.n_ins)
    ...
    for data in data_list:
        dataset = data[0]
        data_name = data[1]
        save_path = save_direc + f'/solution_{data_name}{config.max_solve_time}s.npy'
        ...
        if (not os.path.exists(save_path)) or config.cover_flag:
            ...
            print(f"path : ./data/{data_source}/{data_name}")
