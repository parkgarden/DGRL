...
class Job:
    def __init__(self, job_num, args, job_id, op_config):
        self.args = args
        self.job_id = job_id
        self.operations = [Operation(self.args, self.job_id, config, job_num) for config in op_config]
...
class Operation:
    def __init__(self, args, job_id, config, job_num):
        self.args = args
        self.job_id = job_id
        ...
        if self.args.graph_len == 0:
            graph_len = len(config)
        else:
            graph_len = self.args.graph_len

        if graph_len - 1 < self.op_id:
            self.distant = 1
        else:
            self.distant = 0
        
        if self.args.dyn_job_arrival:
            if self.job_id >= job_num - self.args.JA_num:
                self.not_arrived = 1
                self.avai_time = self.args.JA_interval * (self.job_id - job_num + self.args.JA_num + 1)
            else:
                self.not_arrived = 0
        
        
        if len(self.machine_and_processtime) == 0:
            self.expected_process_time = 0
        else:
            total = 0
